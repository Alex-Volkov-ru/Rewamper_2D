...
func _on_pressed() -> void:
	...
	talent_popup.set_talent_info("regen", level, self)

# Функция для обновления уровня после покупки
func upgrade_talent():
	level = Global.get_talent("regen", 0)
	if level < 5:
		level += 1
		Global.set_talent("regen", level)  # Обновляем уровень таланта
	...
	skill_upgraded.emit("regen", level)

# 🔄 Функция обновления UI (вызывается при сбросе талантов)
func update_talent_ui():
	level = Global.get_talent("regen", 0)  # Загружаем уровень после сброса
	label.text = str(level) + "/5"
