...
@onready var bullet_wogol_controller: Node = $BulletWogolController
...
@export var health_value: float = 50  # Начальное значение здоровья
...
func shoot_bullet():
	...
	if bullet_wogol_controller == null:
	...
	bullet_wogol_controller.spawn_bullet(global_position, player.global_position)
