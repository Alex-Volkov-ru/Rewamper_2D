...
@onready var bullet_chort_controller: Node = $BulletChortController
...
@export var health_value: float = 25  # Начальное значение здоровья
...
func shoot_bullet():
	...
	if bullet_chort_controller == null:
	...
	bullet_chort_controller.spawn_bullet(global_position, player.global_position)
