...
@onready var bullet_ice_zombie_controller: Node = $BulletIceZombieController
...
@export var health_value: float = 35  # Начальное значение здоровья
...
func shoot_bullet():
	...
	if bullet_ice_zombie_controller == null:
	...
	bullet_ice_zombie_controller.spawn_bullet(global_position, player.global_position)
