...
@onready var bullet_ork_shaman_controller: Node = $BulletOrkShamanController
...
@export var health_value: float = 40  # Начальное значение здоровья
...
func shoot_bullet():
	...
	if bullet_ork_shaman_controller == null:
	...
	bullet_ork_shaman_controller.spawn_bullet(global_position, player.global_position)
