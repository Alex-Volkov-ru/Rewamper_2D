...
@onready var bullet_necromacer_controller: Node = $BulletNecromacerController
...
@export var health_value: float = 120  # Начальное значение здоровья
...
func shoot_bullet():
	...
	if bullet_necromacer_controller == null:
	...
	bullet_necromacer_controller.spawn_bullet(global_position, player.global_position)
